...
_CANDIDATE_COLS = [
    ...
    # Sequence recovery (matches production `protein_ligand_inverse_folding.py`)
    "aar", "aar_pocket", "aar_nonpocket",
    "n_pocket_residues", "n_nonpocket_residues",
    "predicted_sequence", "gen_seconds", "score_seconds",
] + _PLL_VARIANTS
...
def _pocket_mask(target: dict, threshold: float = 5.0) -> torch.Tensor:
    """Boolean [L_full] pocket mask: residues whose CA is within `threshold` Å of any
    GT ligand atom. Matches `ProteinLigandInverseFolding.compute_binding_pocket` in
    `src/lobster/metrics/protein_ligand_inverse_folding.py` (CA-only variant) and the
    pocket convention used by `compute_complex_metrics_vs_gt`.
    """
    valid = target["protein_mask"].bool()
    lig = target.get("ligand_coords")
    if lig is None or lig.numel() == 0:
        return torch.zeros_like(valid)
    ca = target["coords_res"][:, 1, :].detach().cpu().float()
    lig = lig.detach().cpu().float()
    dists = torch.cdist(ca, lig)
    pmask = (dists.min(dim=1).values < threshold) & valid.cpu()
    return pmask


def _aar_from_strings(gt_str: str, pred_str: str, mask_seq: list[bool] | None = None) -> float:
    if not gt_str or len(gt_str) != len(pred_str):
        return float("nan")
    if mask_seq is None:
        n = len(gt_str)
        if n == 0:
            return float("nan")
        return sum(a == b for a, b in zip(gt_str, pred_str)) / n
    n = sum(mask_seq)
    if n == 0:
        return float("nan")
    return sum((a == b) for a, b, m in zip(gt_str, pred_str, mask_seq) if m) / n


def _compute_aar(target: dict, pred_seq_tokens: torch.Tensor, aa_transform) -> tuple[float, float, float, int, int, str]:
    """Sequence-identity AAR vs GT, restricted to valid (mask=1) positions.

    Returns
    -------
    aar_overall, aar_pocket, aar_nonpocket, n_pocket, n_nonpocket, pred_str
    """
    ...
    pmask_full = _pocket_mask(target).bool()                       # [L_full] (CPU)
    pmask_valid = pmask_full[valid.detach().cpu()].tolist()        # length == len(gt_str)
    nonpocket_seq = [(not p) for p in pmask_valid]
    n_pocket = int(sum(pmask_valid))
    n_nonpocket = int(sum(nonpocket_seq))

    aar_overall = _aar_from_strings(gt_str, pred_str)
    aar_pocket = _aar_from_strings(gt_str, pred_str, pmask_valid)
    aar_nonpocket = _aar_from_strings(gt_str, pred_str, nonpocket_seq)
    return aar_overall, aar_pocket, aar_nonpocket, n_pocket, n_nonpocket, pred_str
...
def _generate_one(model, target: dict, *, include_ligand: bool, gen_kwargs: dict):
    ...
    # Joint encode protein+ligand to match production IF
    # (`src/lobster/metrics/protein_ligand_inverse_folding.py:_evaluate_single_design`),
    # which uses `model.encode_protein_ligand_structure` and forwards both
    # ligand structure tokens AND continuous embeddings as fixed context.
    input_lig_atom = None
    ...
    if include_ligand:
        ...
        input_lig_struc_embeds = enc.get("ligand_embeddings")
...
def main() -> None:
    ...
    for ti, pdb_id in enumerate(pdb_ids):
        ...
        for ci in range(args.N):
            ...
            try:
                (aar, aar_pocket, aar_nonpocket,
                 n_pocket, n_nonpocket, pred_str) = _compute_aar(target, pred_seq, aa_transform)
            except Exception as e:
            ...
            row = {
                ...
                "aar": aar,
                "aar_pocket": aar_pocket,
                "aar_nonpocket": aar_nonpocket,
                "n_pocket_residues": n_pocket,
                "n_nonpocket_residues": n_nonpocket,
