...
DATA_DIR = f"{LOBSTER_DIR}/data/posebusters/processed/posebusters_benchmark_no_overlap/"
RAW_DATA_DIR = f"{LOBSTER_DIR}/data/posebusters/posebusters_benchmark_set/"
...
GENUME_PARAMS = {
    ...
    "temperature_struc": 0.186,
    "temperature_ligand": 0.2,
    "stochasticity_seq": 10,
    "stochasticity_struc": 10,
    "stochasticity_ligand": 10,
    "nsteps": 200,
    "length": 100,
}
...
def get_ligand_ids():
    ...
    return sorted(f.stem.replace("_ligand", "") for f in Path(DATA_DIR).glob("*_ligand.pt"))


def get_smiles(ligand_id: str) -> str:
    """Get canonical SMILES for a ligand from its SDF file."""
    from rdkit import Chem, RDLogger
    RDLogger.DisableLog("rdApp.*")
    sdf_path = f"{RAW_DATA_DIR}/{ligand_id}/{ligand_id}_ligand.sdf"
...
def run_genume(checkpoint: str, ligand_id: str, num_designs: int, output_dir: str):
    """Generate proteins with Gen-UME, validate with ESMFold, score with RF3.

    Pipeline per ligand:
      1. Load Gen-UME checkpoint and ESMFold model
      ...
      4. Run RF3 co-folding on (sequence, SMILES) → compute ipTM, pTM
      5. Save all structures (decoded PDB, RF3 PDB) and metrics

    Generation uses the best hyperparameters from the sweep (GENUME_PARAMS):
      temp_ligand=0.2, stochasticity_ligand=10, nsteps=200, length=100.
    ESMFold is skipped — RF3 provides the independent validation.
    ...
    # RF3 scoring
    logger.info(f"Running RF3 on {len(results_df)} designs")
    rf3_rows = []
    for _, row in results_df.iterrows():
        name = f"{ligand_id}_d{int(row['design_idx'])}"
        seq = row.get("sequence", "")
        if not seq or not isinstance(seq, str):
            rf3_rows.append({"rf3_error": "no_sequence"})
            continue
        logger.info(f"  RF3: {name}")
        rf3_rows.append(run_rf3(name, seq, smiles, output_dir))

    combined = pd.concat([results_df.reset_index(drop=True), pd.DataFrame(rf3_rows)], axis=1)
...
def run_proteina(ligand_id: str, num_designs: int, output_dir: str):
    """Generate binders with Proteina-Complexa, score with RF3.
    ...
    for d in range(num_designs):
        ...
        logger.info(f"Proteina design {d+1}/{num_designs}: {run_name}")
        ...
        logger.info(f"  RF3: {name} (len={len(seq_str)})")
        rf3_metrics = run_rf3(name, seq_str, smiles, output_dir)
...
def main():
    ...
    p = sub.add_parser("submit", help="Submit SLURM jobs for all ligands × both models")
    ...
    p = sub.add_parser("run_genume", help="Run Gen-UME on a single ligand")
    p.add_argument("--checkpoint", required=True)
    p.add_argument("--ligand_id", required=True)
    p.add_argument("--num_designs", type=int, default=5)
    p.add_argument("--output_dir", required=True)

    p = sub.add_parser("run_proteina", help="Run Proteina on a single ligand")
    p.add_argument("--ligand_id", required=True)
    p.add_argument("--num_designs", type=int, default=5)
    p.add_argument("--output_dir", required=True)

    p = sub.add_parser("run_genume_filtered", help="Gen-UME: generate many → ESMFold filter → RF3 top-K")
    p.add_argument("--checkpoint", required=True)
    p.add_argument("--ligand_id", required=True)
    p.add_argument("--num_designs", type=int, default=111, help="Total designs to generate")
    p.add_argument("--rf3_top_k", type=int, default=5, help="Number of top designs to send to RF3")
    p.add_argument("--output_dir", required=True)

    p = sub.add_parser("submit_filtered", help="Submit filtered benchmark: Gen-UME (ESMFold→RF3) vs Proteina")
    p.add_argument("--checkpoint", required=True, help="Gen-UME checkpoint path")
    p.add_argument("--num_designs", type=int, default=111, help="Gen-UME designs to generate per ligand")
    p.add_argument("--rf3_top_k", type=int, default=5, help="Top-K designs to send to RF3")
