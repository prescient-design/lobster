...
from prescient_plm.model.openfold_utils import backbone_loss
from prescient_plm.transforms import StructureFeaturizer
...
def _get_model_output(example_aa_sequence, device):
    ...
    tokenized_input = m.tokenizer(
        [example_aa_sequence], return_tensors="pt", add_special_tokens=False
    )
    with torch.no_grad():
