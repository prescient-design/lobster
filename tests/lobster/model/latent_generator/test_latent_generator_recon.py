...
# Test data paths
TEST_DATA_DIR = Path(__file__).parents[4] / "src" / "lobster" / "model" / "latent_generator" / "example" / "example_pdbs"
...
class TestProteinOnlyModels:
    ...
    def test_protein_encoding_decoding(self, model_name, protein_data):
        ...
        mask = torch.ones(decoded_outputs.shape[0], decoded_outputs.shape[1], dtype=torch.bool, device=decoded_outputs.device)
...
class TestProteinLigandModels:
    ...
    def test_protein_ligand_encoding_decoding(self, model_name, protein_ligand_data):
        ...
        output_path = f"test_decoded_{model_name.replace(' ', '_')}.pdb"
        writepdb_ligand_complex(output_path, 
        ...
        protein_ligand_data = {k: v.to(decoded_outputs["protein_coords"].device) for k, v in protein_ligand_data.items() if isinstance(v, torch.Tensor)}
...
class TestLigandOnlyModels:
    ...
    def test_ligand_encoding_decoding(self, model_name, ligand_data):
        ...
        # Calculate the RMSD between the original and decoded structures
        ligand_data = {k: v.to(decoded_outputs["ligand_coords"].device) for k, v in ligand_data.items() if isinstance(v, torch.Tensor)}
