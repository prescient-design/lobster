...
def patched_generate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        generate_mod, "resolve_checkpoint", lambda v: Path("/dev/null")
    )
    ...
    monkeypatch.setattr(
        generate_mod.hydra.utils, "instantiate", lambda cfg, **kw: fake_model
    )

    # Patch ESMFold loading — never instantiate the 600M weights.
    monkeypatch.setattr(
        generate_mod, "LobsterPLMFold", MagicMock(return_value=MagicMock())
    )
...
def test_generate_routes_to_correct_helper(
    ...
    assert cfg.generation.mode == expected_mode, (
        f"{config_name} composed with generation.mode={cfg.generation.mode}; "
    ...
    assert called == [expected_helper], (
        f"dispatcher routing for {config_name}: expected ONLY "
        f"{expected_helper!r} to fire, but {called} fired."
    )
...
def test_unknown_mode_raises(patched_generate, tmp_path) -> None:
    ...
    cfg = _compose_with_temp_output(
        "experiment/generate_unconditional", tmp_path
    )
