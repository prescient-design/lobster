...
from prescient_plm.data import DataFrameDatasetInMemory
from prescient_plm.model._utils import model_typer
from prescient_plm.tokenization import PmlmTokenizerTransform
...
def predict(dataframe: pd.DataFrame, config: DictConfig) -> pd.DataFrame:
    ...
    tokenizer_path = importlib.resources.files("prescient_plm") / "assets" / config.tokenizer
