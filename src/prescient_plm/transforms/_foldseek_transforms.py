...
from prescient_plm.model import PrescientPLMFold
...
class FoldseekTransform(Transform):
    ...
    def __init__(
        self,
        foldseek: str,
        pplm_fold_model_name: str = "esmfold_v1",
        linker_length: int = 25,
    ):
        super().__init__()
        assert os.path.exists(foldseek), f"Foldseek not found: {foldseek}"
        self._foldseek = foldseek
        self._pplm_fold_model_name = pplm_fold_model_name
        self._linker_length = linker_length
        self._model = PrescientPLMFold(model_name=self._pplm_fold_model_name)
        self._model.eval()
        self._model.model.trunk.set_chunk_size(64)

    def transform(self, sequences: list[str], chains: list = None) -> dict:

        pdb_file = self._pplm_fold_transform(sequences)
    ...
    def _pplm_fold_transform(self, sequences: list[str]) -> str:
