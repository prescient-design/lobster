...
def add_gaussian_noise(tensor, mean=0.1, std=1.0, device="cpu"):
...
class EarlyStopping:
    ...
    def __init__(self, path, patience=7, verbose=False, delta=0, use_multi_gpu=True):
    ...
    def _save_checkpoint(self, model, path, append=""):

        model_name = path.split("/")[-1]
        itr = model_name.split("_")[-1]

        if torch.cuda.device_count() > 1 and self.use_multi_gpu:
            torch.save(model.module.state_dict(), self.loc + model_name + append + ".pth")
        else:
            torch.save(model.state_dict(), self.loc + model_name + append + ".pth")
    ...
    def save_checkpoint(self, val_loss, model, path):
        ...
        self._save_checkpoint(model, path, append="_best_valid")
