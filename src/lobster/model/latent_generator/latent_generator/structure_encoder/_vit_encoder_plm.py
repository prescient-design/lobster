...
class PLMEncoder(BaseEncoder):
    ...
    def __init__(self, embed_dim:int, embed_dim_hidden: int, data_fixed_size: int, uvit_n_layers: int, uvit_n_heads: int, uvit_dim_head: int, uvit_position_embedding_type: str,
                 dropout: float = 0.0, attention_dropout: float = 0.0, use_sequential_to_out: bool = False, attn_drop_out_rate: float = 0.0, translation_scale: float = 1.0, use_template_coords: bool = False,
                 *args, **kwargs):
        super().__init__()
    ...
    def forward(
        ...
        coords: Optional[TensorType["b n 3", float]] = None,
        ...
        **kwargs
        ...
        emb = self.vit(plm_embeddings, seq_mask=seq_mask, residue_index=residue_index, coords=coords, attn_drop_out_rate=self.attn_drop_out_rate, return_embeddings=return_embeddings)
