...
#from ._lrf import generate_rotations_by_sign_flips
...
def apply_random_se3(
    coords_in, atom_mask=None, translation_scale=1.0, rotation_mode="svd"
):
    """
...
def transform_to_global_frame(points, transform_only=False, mask =  None, apply_stochastic_fa=False, get_all_frames=False):
    ...
    global_frame, global_mean = compute_global_reference_frame(points_flat, mask, apply_stochastic_fa=apply_stochastic_fa, get_all_frames=get_all_frames)
    ...
    if get_all_frames:
        ...
        if len(points.shape) == 4:
            ...
            transformed_points = transformed_points * mask.unsqueeze(-1).unsqueeze(-1)  # Apply mask to transformed points
        else:
...
def apply_global_frame_to_coords(coords, frame_type="pca_frame", mask=None, apply_stochastic_fa=False, get_all_frames=False):
    """
    ...
    if frame_type == "mol_frame":
        ...
        coords_ = transform_to_global_frame(coords_, transform_only=True, mask=mask, apply_stochastic_fa=apply_stochastic_fa, get_all_frames=get_all_frames)
