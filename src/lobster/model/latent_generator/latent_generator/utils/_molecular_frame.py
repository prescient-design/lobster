...
def _normalize(vec, dim=-1):
    return nan_to_num(
        vec / (torch.norm(vec, dim=dim, keepdim=True) + 1e-8)
    )
...
def create_local_coordinate_system(point1, point2):
    ...
    global_z = torch.tensor([0., 0., 1.], device=point1.device)
    global_y = torch.tensor([0., 1., 0.], device=point1.device)
...
def frags_to_seq(lrfs, lrf_means, grf_coords, non_col_points=None, use_pca=False, just_coord=True):
    ...
    frag_centroid_local_coords, R_m2w, t_m2w = construct_local_frame(lrf_means, non_col_points=grf_coords, use_pca=use_pca)
    ...
    for i, lrf in enumerate(lrfs):
        ...
        frag_atom_local_coords, R_f2w, t_f2w = construct_local_frame(lrf, non_col_points=selected_points, use_pca=use_pca)
...
def get_grf_from_lrf(coords, sequence=None):
    ...
    for i in range(len(coordinate_neighbors_backbone)):
        lrf_n_ca_c_means_i = torch.stack([coordinate_neighbors_backbone[i][:,0].mean(axis=0),
    ...
    coord_out_molframe_list, coord_out_locframe_list, features_lrf, R_m2w, t_m2w = frags_to_seq(coordinate_neighbors_backbone, lrf_means, grf_coords, non_col_points=lrf_n_ca_c_means)
...
def get_grf_from_lrf_from_pdb(pdb_path, save_path, device):
    ...
    coords_grf, coords_lrf, features_lrf, seq_lrf, indices_lrf = get_grf_from_lrf(backbone_coords.clone(), pt_file['sequence'])
