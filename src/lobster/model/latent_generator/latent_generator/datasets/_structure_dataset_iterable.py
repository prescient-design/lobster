...
class ShardedStructureDataset(IterableDataset):
    ...
    def _create_shard_order(self, gpu_rank: int = 0, worker_id: int = 0) -> List[int]:
        ...
        if self.shuffle_shards:
            ...
            logger.debug(f"GPU {gpu_rank}, Worker {worker_id} shuffled shard order: {shard_order[:5]}... (showing first 5)")
            return shard_order
        else:
            shard_order = list(range(self.num_shards))
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        ...
        if is_distributed:
            ...
            end_shard_order_idx = start_shard_order_idx + shards_per_worker if worker_id < num_workers - 1 else self.num_shards
            ...
            logger.debug(f"Worker {worker_id} start_shard_order_idx: {start_shard_order_idx}, end_shard_order_idx: {end_shard_order_idx}")
        ...
        if num_workers == 1:
            ...
            self._prefetch_thread = threading.Thread(
                ...
                args=(worker_shard_indices, shard_buffer, shared_state)
            )
            ...
            if worker_shard_indices:
                ...
                for shard_idx in worker_shard_indices:
                    ...
                    if first_shard:  # Successfully loaded a non-empty shard
                        ...
                        logger.debug(f"GPU {gpu_rank}, Worker {worker_id} loaded initial shard {shard_idx} with {len(first_shard)} structures")
                        break
                else:
                    logger.warning(f"GPU {gpu_rank}, Worker {worker_id} could not load any valid shards from {worker_shard_indices}")
        ...
        try:
            ...
            while True:
                ...
                if self.max_batches_per_epoch is not None and batches_yielded >= self.max_batches_per_epoch:
                    logger.debug(f"GPU {gpu_rank}, Worker {worker_id} reached batch limit ({self.max_batches_per_epoch}), ending epoch")
                    break
                ...
                if num_workers > 1:
                    if not shard_buffer:
                        ...
                        shard_loaded = False
                        logger.debug(f"GPU {gpu_rank}, Worker {worker_id} loading shard {current_shard_idx} of {len(worker_shard_indices)}")
                        while current_shard_idx < len(worker_shard_indices) and not shard_loaded:
                            ...
                            current_shard_idx += 1
                            logger.debug(f"GPU {gpu_rank}, Worker {worker_id} loading shard {next_shard_idx}, current_shard_idx {current_shard_idx}")
                            next_shard = self._load_shard(next_shard_idx)
                            
                            logger.debug(f"GPU {gpu_rank}, Worker {worker_id} loaded shard {next_shard_idx}, got {len(next_shard)} structures")
                            ...
                            if next_shard:  # Successfully loaded a non-empty shard
                                ...
                                logger.debug(f"GPU {gpu_rank}, Worker {worker_id} shard {next_shard_idx} was empty/corrupted, trying next shard")
                        
                        if not shard_loaded:
                            # No more valid shards available
                            logger.debug(f"GPU {gpu_rank}, Worker {worker_id} tried all {len(worker_shard_indices)} assigned shards but found no valid ones")
                            logger.debug(f"GPU {gpu_rank}, Worker {worker_id} assigned shards were: {worker_shard_indices}")
                            break
...
if __name__ == "__main__":
    ...
    from lobster.model.latent_generator.latent_generator.datamodules._utils import collate_fn_backbone
    from lobster.model.latent_generator.latent_generator.datasets._transforms import StructureBackboneTransform
    from lobster.model.latent_generator.latent_generator.io import writepdb
    from icecream import ic
    import tqdm
    ...
    for num_gpus, description in gpu_configs:
        ...
        try:
            ...
            for gpu_rank in range(num_gpus):
                ...
                if overlap > 0:
                    logger.warning(f"    WARNING: GPU {gpu_rank} has {overlap} overlapping structures with previous GPUs")
