...
from lobster.model.latent_generator.latent_generator.utils import apply_random_se3_batched, get_zernike_descriptors, calculate_geometric_features
from lobster.model.latent_generator.latent_generator.utils.mini3di import Encoder, calculate_cb
from lobster.model.latent_generator.latent_generator.utils._kinematics import xyz_to_c6d, c6d_to_bins
...
class ESMEmbeddingTransform(BaseTransform):
    ...
    def __init__(self, model_name: str = "esmc_300m", device: str = "auto", **kwargs):
        ...
        logger.info(f"ESM model loaded successfully")
    ...
    def sequence_to_esm_embeddings(self, sequence: torch.Tensor, chains: torch.Tensor = None) -> torch.Tensor:
        ...
        if sequence.dim() == 1:
            ...
            if chains is not None:
                ...
                if len(unique_chains) > 1:
                    ...
                    if len(unique_chains) == 1:
                        ...
                        logger.warning(f"Only one chain found, using the same chain for both chains")
                    ...
                    embeddings = self.model.logits(protein_tensor, LogitsConfig(sequence=True, return_embeddings=True)).embeddings
                    ...
                    combined_chain_1_mask = np.concatenate([chain_1_mask[:index_to_add], np.zeros(40, dtype=bool), chain_1_mask[index_to_add:]])
                    combined_chain_2_mask = np.concatenate([chain_2_mask[:index_to_add], np.zeros(40, dtype=bool), chain_2_mask[index_to_add:]])
                    ...
                    if embeddings.shape[1] != len(sequence):
                        raise ValueError(f"Embeddings shape {embeddings.shape} does not match sequence length {len(sequence)}")
            ...
            for i in range(sequence.shape[0]):
                seq_embeddings = self.sequence_to_esm_embeddings(
                    sequence[i], 
                    chains[i] if chains is not None else None
                )
            ...
            if len(x["indices"]) > self.max_length:
                ...
                if len(chain_indices) > self.max_length:
                    ...
                    if "esm_c_embeddings" in x:
                        ...
                        x["plm_embeddings"] = torch.tensor(x["plm_embeddings"], dtype=x["coords_res"].dtype, device=x["coords_res"].device)
                else:
...
class StructureZernikeTransform(BaseTransform):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def format_loaded_pdb(self, x):
        ca_coords = x['coords_res'][:,1,:]  # Get CA coordinates
        #turn into numpy array
        ca_coords = ca_coords.numpy()
        # Convert integer sequence to one-letter codes using residue_constants
        seq_1letter = residue_constants.aatype_to_str_sequence(x['sequence'])
        # Convert single letter codes to three letter codes
        seq_3letter = [self.AA_1_TO_3[aa] for aa in seq_1letter]
        #turn into numpy array
        seq_3letter = np.array(seq_3letter)
        return ca_coords, seq_3letter

    def __call__(self, x: dict) -> dict:
        ca_coords, seq_3letter = self.format_loaded_pdb(x)
        zm_pdb = get_zernike_descriptors(ca_coords, seq_3letter)
        geo_features = calculate_geometric_features(ca_coords, device=ca_coords.device)
        x["zernlike_descriptors"] = zm_pdb
        x["geometric_features"] = geo_features
        return x
...
class BinderTargetTransform(BaseTransform):
    ...
    def __call__(self, batch: dict) -> dict:
        ...
        conditioning_res_tensor_mask, conditioning_res_tensor_mask_2 = self.make_conditioning_res_tensor_mask(chains, mask)
    ...
    def __call__(self, x: dict) -> dict:
        ...
        for i in range(len(x["sequence"])):
            ...
            if "atoms_channel_ltr" not in x:
                x["atoms_channel_ltr"] = np.array([residue_constants.HASH_2_ELEMENT_FUNCBIND[int(x["atoms_channel"][i])] for i in range(len(x["atoms_channel"]))])
