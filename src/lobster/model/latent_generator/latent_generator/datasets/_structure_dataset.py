...
#TODO simplify this class to only have the necessary methods for the dataset
...
from lobster.model.latent_generator.latent_generator.datasets._sampler import RandomizedMinorityUpsampler
import pandas as pd
import os
import pickle
...
class StructureDataset(Dataset):
    ...
    def __init__(
        ...
        cluster_file: Union[str, os.PathLike]=None,
        transform=None,
        pre_transform=None,
        ...
        min_len: int = 100,
        testing: bool = False,
        files_to_keep: Union[str, os.PathLike]=None,
        use_mmap: bool = False,
    ):
        ...
        self._load_data()
        logger.info(f"Loaded data points.")
        #breakpoint()
    ...
    def processed_file_names(self) -> list[str]:
        ...
        return [
            f"{self.dataset_filenames[i]}"
            for i, f in enumerate(self.dataset_filenames)
        ]
    ...
    def _load_data(self):
        ...
        process_args = [(f, files_to_keep, self.cluster_dict if self.cluster_file is not None else None) 
                       for f in all_files]
        ...
        if not self.load_to_disk:
            ...
            if self.use_mmap:
                ...
                logger.info(f"Loaded dataset with memory mapping")
            else:
                self.preloaded_dataset = torch.load(self.load_to_disk_file)
                logger.info(f"Loaded dataset into memory")
            
            logger.info(f"Turning to df...")
