...
from lobster.model.latent_generator.latent_generator.datasets import StructureBackboneTransform
...
class DSSPLinearProbe(lightning.Callback):
    ...
    def __init__(
        self,
        token_dim: int,
        train_paths: str = "/data/lisanzas/structure_tokenizer/studies/data/pinder/processed_atom/pdb/train_dssp/",
        val_paths: str = "/data/lisanzas/structure_tokenizer/studies/data/pinder/processed_atom/pdb/val_dssp/",
    ...
    def _train_epoch(self, pl_module: lightning.LightningModule) -> float:
        ...
        for batch in self.train_batches:
            ...
            pl_module.log_dict({'dssp_probe_train_loss': loss.item(), 'dssp_probe_train_acc': acc.item()}, batch_size=tokens_batch.shape[0])
    ...
    def _validate(self, pl_module: lightning.LightningModule) -> float:
        ...
        with torch.no_grad():
            for batch in self.val_batches:
                ...
                pl_module.log_dict({'dssp_probe_val_loss': loss.item(), 'dssp_probe_val_acc': acc.item()}, batch_size=tokens_batch.shape[0])
    ...
    def on_train_batch_end(
        ...
        pl_module.log_dict({
            'dssp_probe_train_acc': best_train_acc,
            'dssp_probe_val_acc': best_val_acc
