...
def generate_rotations_by_sign_flips(rot_matrices):
    ...
    sign_flips = torch.tensor(
        [
            [1.0, 1.0, 1.0],  # No flips
            [-1.0, -1.0, 1.0],  # Flips in axes 0 and 1
            [-1.0, 1.0, -1.0],  # Flips in axes 0 and 2
            [1.0, -1.0, -1.0],  # Flips in axes 1 and 2
        ],
        device=device,
    )
...
def apply_stochastic_fa(lrf, give_all=False):
    ...
    random_indices = torch.randint(
        0, lrf_expanded.shape[2], (lrf_expanded.shape[0], lrf_expanded.shape[1]), device=lrf.device
    )  # (B, L)
    # Select the random frames
    selected_frames = lrf_expanded[
        torch.arange(lrf_expanded.shape[0]).unsqueeze(1),
        torch.arange(lrf_expanded.shape[1]).unsqueeze(0),
        random_indices,
    ]  # (B, L, 3, 3)
...
def get_non_zero_neighbors_fast(lrf_points_nb, neighbor_mask, max_neighbors):
    ...
    mask_indices = torch.where(neighbor_mask, mask_indices, torch.tensor(L, device=device))
...
def compute_geometric_features(
    points, radius=30.0, max_neighbors=30, stochastic_fa=True, mask=None, mask_probability=0.8, give_all_frames=False
):
    """
    ...
    if give_all_frames:
        ...
        for frame in range(lrf_points_nb.shape[3]):
            lrf_points_nb_list.append(
                get_non_zero_neighbors_fast(lrf_points_nb[:, :, :, frame, :], neighbor_mask, max_neighbors)
            )
