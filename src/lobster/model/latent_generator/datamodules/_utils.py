    ...
    for bb_dict in batch:
        ...
        padded_sequence.append(
            torch.cat(
                [
                    bb_dict['sequence'],
                    torch.full((max_length - bb_dict['sequence'].shape[0],), residue_constants.PEPTIDE_ALPHABET.index("-"), dtype=bb_dict['sequence'].dtype),
                ],
                dim=0,
            )
        )
        ...
        if 'template_coords' in batch[0]:
            padded_template_coords.append(
                torch.cat(
                    [bb_dict['template_coords'], torch.zeros(max_length - bb_dict['template_coords'].shape[0], *bb_dict['template_coords'].shape[1:])],
                    dim=0,
                )
            )
            padded_template_mask.append(
                torch.cat(
                    [bb_dict['template_mask'], torch.zeros(max_length - bb_dict['template_mask'].shape[0], *bb_dict['template_mask'].shape[1:])],
                    dim=0,
                )
            )
            ...
            padded_3di_descriptors.append(
                torch.cat(
                    [
                        ...
                        torch.zeros(max_length - bb_dict['3di_descriptors'].shape[0], *bb_dict['3di_descriptors'].shape[1:]),
                    ],
                    dim=0,
                )
            )
        ...
        if 'plm_embeddings' in batch[0]:
            padded_plm_embeddings.append(
                torch.cat(
                    [bb_dict['plm_embeddings'], torch.zeros(max_length - bb_dict['plm_embeddings'].shape[0], *bb_dict['plm_embeddings'].shape[1:])],
                    dim=0,
                )
            )
    out = {
...
def collate_fn_ligand(batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
    ...
    if 'radius_of_gyration' in batch[0]:
        out['radius_of_gyration'] = torch.tensor([atom_dict['radius_of_gyration'] for atom_dict in batch], dtype=torch.float32)
    
    # Handle SASA property
    if 'solvent_accessible_surface_area' in batch[0]:
        out['solvent_accessible_surface_area'] = torch.tensor([atom_dict['solvent_accessible_surface_area'] for atom_dict in batch], dtype=torch.float32)
