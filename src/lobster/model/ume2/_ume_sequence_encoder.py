    ...
    def forward(
        ...
        if self.auxiliary_tasks is not None and return_auxiliary_tasks:
            for task_name, task_head in self.auxiliary_tasks.items():
                print(f"Trying to do this forward pass for task {task_name}")
                ...
                print(f"CLS token embeddings shape: {cls_embedding.shape}")

                output[task_name] = task_head(cls_embedding)
