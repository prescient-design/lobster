    ...
    def __init__(
        ...
        if task_config.activation.lower() == "swiglu":
            ...
            for i, hidden_size in enumerate(hidden_sizes):
                ...
                swiglu_factory = get_activation_function(
                    "swiglu", 
            ...
            # Output layer
            layers.append(nn.Linear(current_dim, task_config.output_dim))
...
class MultiTaskHead(nn.Module):
    ...
    def __init__(
        ...
        self.task_heads = nn.ModuleDict({
            config.name: TaskHead(input_dim, config, encoder_config)
...
class FlexibleEncoderWithHeads(nn.Module):
    ...
    def __init__(
        ...
        if task_configs is not None:
            self.task_head = MultiTaskHead(hidden_size, task_configs, encoder.config if hasattr(encoder, 'config') else None)
            self.task_configs = {config.name: config for config in task_configs}
        else:
            self.task_head = None
            self.task_configs = {}
