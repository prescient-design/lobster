...
class LobsterMGM(pl.LightningModule):
    def __init__(
        self,
        model_name: str = None,
        lr: float = 1e-3,
        ...
        elif (self._tokenizer_dir is not None) and (self._tokenizer_dir == "mgm_tokenizer"):
            path = importlib.resources.files("lobster") / "assets" / self._tokenizer_dir
            self._transform_fn = transform_fn or MgmTokenizerTransform(
                path, padding="max_length", truncation=True, max_length=self._max_length
            )
            self.tokenizer = MgmTokenizer.from_pretrained(path, do_lower_case=False)
    ...
    def training_step(self, batch, batch_idx):
        if self._tokenizer_dir == "mgm_tokenizer":
            loss, *logging_dicts = self._compute_mgm_loss(batch)
        else:
    ...
    def _compute_mgm_loss(self, batch, x0: float = 0.2, x1: float = 0.8):
        # TODO: pass x0 and x1 from hydra
        # torch.cuda.empty_cache()
        batch, _targets = batch  # targets are the FASTA IDs

        # 1st training objective
        labels_0 = batch["labels_0"].squeeze(1)
        masked_toks_0 = batch["masked_ids_0"].squeeze(1)
        output_0 = self.model(
            input_ids=masked_toks_0,
            attention_mask=batch["attention_mask"].squeeze(1),
            labels=labels_0,
        )
        loss_0 = output_0["loss"]
        del masked_toks_0, labels_0, output_0

        # 2nd training objective
        labels_1 = batch["labels_1"].squeeze(1)
        masked_toks_1 = batch["masked_ids_1"].squeeze(1)
        output_1 = self.model(
            input_ids=masked_toks_1,
            attention_mask=batch["attention_mask"].squeeze(1),
            labels=labels_1,
        )
        loss_1 = output_1["loss"]
        del masked_toks_1, labels_1, output_1

        loss = x0 * loss_0 + x1 * loss_1
        logging_dicts = []
