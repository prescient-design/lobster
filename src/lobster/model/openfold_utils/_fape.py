...
def within_residue_violations(
    ...
    dists_masks = (
        atom14_atom_exists[..., :, :, None] * atom14_atom_exists[..., :, None, :] * dists_masks
    )
...
def find_structural_violations(
    ...
    **kwargs,
    ...
    restype_atom14_bounds = make_atom14_dists_bounds(
...
def compute_renamed_ground_truth(
    ...
    ) * atom14_gt_exists + alt_naming_is_better[..., None] * batch["atom14_alt_gt_exists"]
...
class AlphaFoldLoss(nn.Module):
    ...
    TODO(amyxlu,11/09/2023): this bundled-loss class needs to be cleaned up to work with
    current PPLMFold codebase; use fape_loss for now."""
    ...
    def forward(self, out, batch, _return_breakdown=False):
        ...
        loss_fns = {
            "distogram": lambda: distogram_loss(
                logits=out["sm"]["distogram_logits"],
                **{**batch, **asdict(self.config.distogram)},
            ),
            "experimentally_resolved": lambda: experimentally_resolved_loss(
                logits=out["sm"]["experimentally_resolved_logits"],
                **{**batch, **asdict(self.config.experimentally_resolved)},
            ),
            ...
            "plddt_loss": lambda: lddt_loss(
                ...
                **{**batch, **asdict(self.config.plddt_loss)},
            ),
            "masked_msa": lambda: masked_msa_loss(
                logits=out["masked_msa_logits"],
                **{**batch, **asdict(self.config.masked_msa)},
            ),
            "supervised_chi": lambda: supervised_chi_loss(
                out["sm"]["angles"],
                out["sm"]["unnormalized_angles"],
                **{**batch, **asdict(self.config.supervised_chi)},
            ),
        ...
        if self.config.tm.enabled:
            loss_fns["tm"] = lambda: tm_loss(
                ...
                **{**batch, **out, **asdict(self.config.tm)},
            )

        cum_loss = 0.0
        losses = {}
        for loss_name, loss_fn in loss_fns.items():
            weight = getattr(self.config, loss_name).weight
            loss = loss_fn()
...
def make_default_alphafold_loss():
    # TODO(amyxlu): update this to be hydra-configurable for hparam sweep
    cfg = LossConfig()
    return AlphaFoldLoss(cfg)