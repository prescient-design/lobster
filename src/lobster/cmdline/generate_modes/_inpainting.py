"""LeFlur inpainting mode.

Generates new sequence/structure content within a fixed structural scaffold.
Accepts per-residue inpainting masks for both the sequence stream and the
structure stream, allowing for motif scaffolding, loop redesign, and partial
diffusion workflows. Optionally validates inpainted regions with ESMFold.

Called from :func:`lobster.cmdline.generate.generate` when
``cfg.generation.mode == "inpainting"``.
"""
...
from ._shared import _validate_with_esmfold
