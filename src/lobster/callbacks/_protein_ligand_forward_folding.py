"""Callback for evaluating ligand-conditioned forward folding on protein-ligand complexes.

This callback compares forward folding (sequence → structure) with and without
ligand context to determine if ligand information improves structure prediction,
...
from lobster.metrics.protein_ligand.forward_folding import ProteinLigandForwardFoldingEvaluator


class ProteinLigandForwardFoldingCallback(lightning.Callback):
    """Callback for evaluating ligand-conditioned forward folding on protein-ligand complexes.

    Compares:
    - Forward folding with protein sequence only
    - Forward folding with protein sequence + ligand context

    Hypothesis: Ligand context should improve pocket structure prediction (lower RMSD).
    ...
    def __init__(
        ...
        nsteps: int = 100,
        metric_prefix: str = "protein_ligand_forward_folding",
    ...
    def setup(self, trainer: lightning.Trainer, pl_module: lightning.LightningModule, stage: str):
        ...
        # Create evaluator
        self._evaluator = ProteinLigandForwardFoldingEvaluator(
        ...
        try:
            ...
            logger.info(f"Loaded {len(self._samples)} protein-ligand test samples for forward folding evaluation")
    ...
    def on_train_batch_end(
        ...
        logger.info(f"Running protein-ligand forward folding evaluation at step {current_step}")

        # Run evaluation (no try-catch to get full traceback for debugging)
        ...
        # Log key insights
        tm_delta = summary.get("mean_tm_score_delta", 0)
        rmsd_pocket_delta = summary.get("mean_rmsd_pocket_delta", 0)
        rmsd_overall_delta = summary.get("mean_rmsd_overall_delta", 0)

        logger.info(
            f"Protein-Ligand Forward Folding (step {current_step}):\n"
            f"  TM-score: {summary['mean_tm_score_no_ligand']:.3f} → "
            f"{summary['mean_tm_score_with_ligand']:.3f} "
            f"(Δ={tm_delta:+.3f})\n"
            f"  Pocket RMSD: {summary['mean_rmsd_pocket_no_ligand']:.2f} Å → "
            f"{summary['mean_rmsd_pocket_with_ligand']:.2f} Å "
            f"(Δ={rmsd_pocket_delta:+.2f} Å)\n"
            f"  Overall RMSD: {summary['mean_rmsd_overall_no_ligand']:.2f} Å → "
            f"{summary['mean_rmsd_overall_with_ligand']:.2f} Å "
            f"(Δ={rmsd_overall_delta:+.2f} Å)"
        )
        ...
        if self.structure_path:
            output_file = os.path.join(
                self.structure_path,
                f"protein_ligand_forward_folding_step{current_step}.csv",
            )
