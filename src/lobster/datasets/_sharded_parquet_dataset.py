...
class ShardedParquetDataset(Dataset):
    def __init__(
        self, parquet_dir, percentile_threshold=90, loss_threshold=None, stats_file=None, rank=None, world_size=None
    ):
        """
    ...
    def _load_sequence_metadata(self):
        ...
        for shard_file in self.shard_files:
            ...
            try:
                ...
                df = pd.read_parquet(shard_file, columns=["sequence_id", "position"])
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            df = pd.read_parquet(
                shard_file, filters=[("sequence_id", "=", seq_id), ("loss", "<=", self.loss_threshold)]
            )
...
def collate_variable_length_sequences(batch):
    ...
    batch_indices = torch.cat([torch.full((size,), i, dtype=torch.int64) for i, size in enumerate(batch_sizes)])
...
def create_distributed_dataloader(parquet_dir, percentile_threshold=90, batch_size=32, num_workers=4):
