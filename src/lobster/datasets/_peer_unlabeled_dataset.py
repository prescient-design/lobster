...
class PEERUnlabeledDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        hf_config: Literal['train_only', 'full', 'protein_only', 'ligand_only', 'function_prediction', 'protein_ligand', 'medium_length'] | None = 'train_only',
    ):
        super().__init__()
    ...
    def _extract_sequences(self, data: pd.DataFrame, task: PEERTask) -> List[tuple]:
        ...
        for col in input_cols:
            ...
            logger.debug(f"Column {col}: sequence_type={sequence_type}, in_include_types={sequence_type in self.include_sequence_types if sequence_type else False}")
