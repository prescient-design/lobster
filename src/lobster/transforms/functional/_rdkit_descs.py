...
# NOTE: This is a hack to get the CDF of the distribution. In the future, should use the new
# Distribution class (via `scipy.stats.make_distribution`) instead
class SupportsCDF(Protocol):
    def cdf(self, x: float | ArrayLike) -> float | np.ndarray: ...


DESCRIPTOR_DISTRIBUTIONS: dict[str, tuple[SupportsCDF, tuple[float, ...]]] = {
    "VSA_EState1": (
...
DESCRIPTOR_DISTRIBUTIONS = MappingProxyType(DESCRIPTOR_DISTRIBUTIONS)  # type: ignore


def smiles_to_rdkit_descs(smiles_seq: str) -> list[float] | None:
    """Get RDKit descriptors for a SMILES sequence.
...
def smiles_to_normalized_rdkit_descs(smiles_seq: str, invert: bool = False) -> list[float] | None:
    r"""Get normalized RDKit descriptors for a SMILES sequence.
    ...
    for name, x in descs.items():
        try:
            dist, (x_min, x_max, *_) = DESCRIPTOR_DISTRIBUTIONS[name]
        except KeyError:
            continue
