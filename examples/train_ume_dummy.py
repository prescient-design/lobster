...
from lobster.model import UMELightningModule
...
class DummyProteinDataset(UMEStreamingDataset):
    ...
    SPLITS = {Split.TRAIN: "/Users/zadorozk/Desktop/lobster/examples/dev/ume/data"}
...
def main():
    ...
    model = UMELightningModule(
        model_name="mini",
        max_length=32,
        lr=1e-4,
        contrastive_loss_weight=0.5,  # Use both MLM and contrastive loss
        use_flash_attn=False,
        contrastive_loss_type="symile",
    )

    # Debug: Check model parameters and gradients
    logger.info("Checking model parameters and gradients:")
    for name, param in model.named_parameters():
        logger.info(f"{name}: requires_grad={param.requires_grad}, device={param.device}")
    
    # Debug: Check model mode
    ...
    logger.info("Model state before training:")
    logger.info(f"Model training mode: {model.training}")
        ...
        for batch in dataloader:
            ...
            for key, value in batch.items():
                if isinstance(value, torch.Tensor):
                    logger.info(f"{key}: shape={value.shape}, requires_grad={value.requires_grad}, device={value.device}")
