...
#SBATCH --mem=256G
#SBATCH --job-name=rest-pc-only
#SBATCH -t 7-00:00:00
#SBATCH -o /cv/scratch/u/lisanzas/slurm_logs/rest_proteina_only_%j.out
#SBATCH -e /cv/scratch/u/lisanzas/slurm_logs/rest_proteina_only_%j.err

# ReST Proteina-only distillation
# Same base checkpoint as i1/i2, but trained only on Proteina-generated designs
# (no Gen-UME self-generated data — pure distillation from Proteina-Complexa)
...
export LOBSTER_RUNS_DIR="/cv/scratch/u/lisanzas/rest_finetune_proteina_only/runs/"
...
uv run lobster_train \
    ...
    data=structure_ligand_rest_proteina_only \
    ...
    logger.name=rest_proteina_only_const_lr1e-5 \
