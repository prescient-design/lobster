...
#SBATCH --mem=128G
#SBATCH -o /cv/scratch/u/lisanzas/proteina_gen_logs/100rep_%A_%a.out
#SBATCH -e /cv/scratch/u/lisanzas/proteina_gen_logs/100rep_%A_%a.err
#SBATCH --job-name=proteina-100rep
#SBATCH -t 05:00:00
#SBATCH --array=0-499
...
RUN_NAME="plinder_100rep_${TASK_NAME}"
...
echo "[RUN] $TASK_NAME (array=$SLURM_ARRAY_TASK_ID, 100 replicas)"
...
if complexa design "configs/${CONFIG_NAME}.yaml" \
    ...
    "++generation.filter.filter_samples_limit=10"; then
    echo "[DONE] $TASK_NAME"
else
