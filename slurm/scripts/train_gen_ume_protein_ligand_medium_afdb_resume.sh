...
#SBATCH --mem=256G
#SBATCH --job-name=gen_ume_pl_med_afdb
#SBATCH -t 7-00:00:00
#SBATCH -q llm

# Gen-UME Protein-Ligand Training Script - MEDIUM MODEL + AFDB SwissProt
#
# Same as the medium resume script but with AFDB SwissProt (198k structures)
# added as additional protein-only training data alongside:
...
#
# Resumes from the medium model sweep checkpoint:
#   - model_size=medium, batch_size=36, lr=1e-3
#   - Checkpoint at step 31567/50000 (epoch 208, val_loss=1.6954)
#   - Effective batch size: 36 * 8 GPUs * 20 accumulate = 5760
#   - SE3 augmentation + ligand minimization enabled
#
# Usage:
#   sbatch slurm/scripts/train_gen_ume_protein_ligand_medium_afdb_resume.sh
...
# Directory setup
export LOBSTER_RUNS_DIR="/cv/scratch/u/lisanzas/gen_ume_protein_ligand_medium_afdb/runs/"
...
mkdir -p /cv/scratch/u/lisanzas/gen_ume_protein_ligand_medium_afdb/runs
...
#CHECKPOINT="/cv/scratch/u/lisanzas/gen_ume_protein_ligand_medium_afdb_scratch/runs/2026-02-21T15-36-15/epoch=51-step=9254-val_loss=1.9894.ckpt"
CHECKPOINT="/cv/scratch/u/lisanzas/gen_ume_protein_ligand_medium_afdb/runs/2026-02-21T15-36-15/epoch=208-step=31567-val_loss=1.6954.ckpt"

# Medium model with sweep settings + AFDB SwissProt data
uv run lobster_train \
    ...
    logger.name=gen_ume_protein_ligand-medium_afdb_resume_bs36_lr1e-3 \
