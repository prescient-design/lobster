#!/bin/bash
#SBATCH --job-name=process_esm_atlas_himem
...
#SBATCH --array=0-2  # 3 array jobs (himem has only 3 nodes available)
#SBATCH --output=/homefs/home/lisanzas/scratch/Develop/lobster/slurm/logs/process_esm_atlas_himem_%A_%a.out
#SBATCH --error=/homefs/home/lisanzas/scratch/Develop/lobster/slurm/logs/process_esm_atlas_himem_%A_%a.err
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=${USER}@gene.com

# NOTE: Using himem partition (only 3 nodes available with 128 vCPU / 1TB mem each)
# Each job processes ~333 directories to maximize utilization of limited himem nodes
...
# With 3 array jobs (himem constraint), each job processes ~333 directories
