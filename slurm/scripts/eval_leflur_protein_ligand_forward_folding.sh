...
#SBATCH -o /cv/scratch/u/lisanzas/slurm_logs/eval_protein_ligand_forward_folding/%J.out
#SBATCH -e /cv/scratch/u/lisanzas/slurm_logs/eval_protein_ligand_forward_folding/%J.err
#SBATCH --mem=128G
#SBATCH --job-name=eval_pl_ff
#SBATCH -t 1-00:00:00
#SBATCH -q llm

# Gen-UME protein-ligand forward folding (co-folding) evaluation on PoseBusters
# Co-folding comparison done separately via SLURM batch (submit_cofold_batch.py)
...
EVAL_PREFIX="${EVAL_PREFIX:-gen_ume_pl_forward_folding}"
OUT_DIR="/cv/scratch/u/lisanzas/evaluations/protein_ligand_benchmarks/${EVAL_PREFIX}_${EVAL_TIMESTAMP}"

mkdir -p /cv/scratch/u/lisanzas/slurm_logs/eval_protein_ligand_forward_folding
mkdir -p "${OUT_DIR}"
